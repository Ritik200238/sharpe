...
export function saveCredentials(credentials: Credentials): void {
  ...
  const file = credentialsPath(credentials.network);
  fs.writeFileSync(file, JSON.stringify(credentials, null, 2));
}

/** Guest JWT — short-lived session token, renewable at any time. */
...
/**
 * Exchange a confirmed on-chain subscribe tx for a long-lived API token.
 * The activation message must be signed by the same wallet that subscribed.
 */
export async function activateApiToken(
  ...
  const token: unknown = response.data?.token ?? response.data;
  if (typeof token !== "string" || token.length === 0) {
    throw new Error(`Activation response missing token: ${JSON.stringify(response.data)}`);
  }
  return token;
}

/** Holds the live auth pair; renews the JWT in place and persists it. */
...
/** GET against the data API; renews the JWT once on 401 and retries. */
export async function apiGet<T = unknown>(
  ...
  try {
    const response = await axios.get<T>(url, { headers: session.headers() });
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      await session.renewJwt();
      const retried = await axios.get<T>(url, { headers: session.headers() });
      return retried.data;
    }
    throw error;
  }
